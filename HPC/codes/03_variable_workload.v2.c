...
#define N_default      10000 // how long is the main array
      ...
      while( first < N )
	{
	 ...
	 #pragma omp task firstprivate(first, last) shared(result) untied
	  ...
	  // region
	  {
	   ...
	   #pragma omp atomic read
	    ...
	    PRINTF("\t thread %d runs chunk %d >> %d for work0 (result is %g, adding %g)\n",
		   me, first, last, current_result, myresult);
	   #endif
	 ...
	 #pragma omp task firstprivate(first, last) shared(result) untied
	  ...
	  // region
	  {
	   ...
	   #pragma omp atomic read
	    current_result = result;
	    PRINTF("\t thread %d runs chunk %d >> %d for work1 (result is %g, adding %g)\n",
		   me, first, last, current_result, myresult);
	   #endif
            #pragma omp atomic update
	    result += myresult;	    
	 ...
	 #pragma omp task firstprivate(first, last) shared(result) untied
	  ...
	  // region
	  {
	   ...
	   #pragma omp atomic read
	    current_result = result;
	    PRINTF("\t thread %d runs chunk %d >> %d for work2 (result is %g, adding %g)\n",
		   me, first, last, current_result, myresult);
	   #endif
            #pragma omp atomic update
	    result += myresult;	    
